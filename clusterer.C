...
void clusterer(){
  ...
  for (int entry=0; entry<lenEntries; entry++) {
    ...
    int eventNumber = eventSummary->eventNumber;
    int run = eventSummary->run;

    double currCart[3];
    geom->getCartesianCoords(eventSummary->peak[0][0].latitude,
			     eventSummary->peak[0][0].longitude,
    ...
    double closestPhi = 999;
    double closestTheta = 999;
    double closestQuad = addQuad(closestPhi,closestTheta);
    int closestEv = -999;

    double currPhi = eventSummary->peak[0][0].phi;
    double currTheta = eventSummary->peak[0][0].theta;
    ...
    for (int entry2=0; entry2<lenEntries; entry2++) {
      ...
      geom->getCartesianCoords(eventSummary->peak[0][0].latitude,
			       ...
			       eventSummary->peak[0][0].altitude,
			       compCart);
      if (cartesianDist(compCart,currCart) > 1500) continue; //also don't want to look at stuff super far away
